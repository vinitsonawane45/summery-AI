...
#         if 'pdf_file' not in request.files or not request.files['pdf_file'] or not request.files['pdf_file'].filename:
#             return jsonify({'error': 'No PDF file uploaded. Please upload a PDF to summarize.'}), 400
            
#         text = extract_text_from_pdf(request.files['pdf_file'])
        
#         if not text or len(text.strip()) < 20:
#             return jsonify({'error': 'Extracted text too short. Please provide a PDF with at least 20 characters of readable content.'}), 400
...
async def summarize():
    try:
        ...
        if not data or 'input' not in data:
            return jsonify({'error': 'No input provided. Please enter text or a URL.'}), 400
            
        input_text = data['input'].strip()
        if not input_text:
            return jsonify({'error': 'Input is empty. Please enter text or a URL.'}), 400
            
        text = input_text
        if urlparse(input_text).scheme in ["http", "https"]:
            text = await fetch_url_content(input_text)
