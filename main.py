...
def analyze_text(text):
    try:
        ...
        clean_text = re.sub(r'[^\w\s]', '', text.lower())  # Normalize text
        words = word_tokenize(clean_text)
        if not words:
...
async def summarize():
    try:
        ...
        data = request.get_json()
        if not data or 'text' not in data:  # Changed 'input' to 'text'
            return jsonify({'error': 'No text provided. Please enter text or a URL.'}), 400
