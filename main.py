    ...
    try:
        ...
        try:
            ...
            for page in reader.pages[:20]:  # Limit to first 20 pages
                ...
                text += page_text + "\n"
                if len(text) > 20000:  # Increased character limit
                    break
                    
            # If no text extracted, try OCR fallback
            if not text.strip():
                app.logger.info("No text extracted with PyPDF2, trying OCR fallback")
                try:
                    images = pdf2image.convert_from_bytes(
                        ...
                        thread_count=4,
                        first_page=1,
                        last_page=5  # Limit to first 5 pages for OCR
                    )
                    ...
                    raise ValueError("Could not extract text (OCR failed)")
